...
export default function Celebrities() {
      ...
      const interval = setInterval(async () => {
        ...
        try {
          ...
          if (outputUrl) {
            ...
            // After we get the final image, we can create & poll Printful mockups
  ...
  // 4) Generate mockups via two-step approach
  const generateMockups = async (finalUrl: string) => {
    ...
    try {
      // (A) Create tasks
      const createRes = await fetch("/api/printful/createMockups", {
        method: "POST",
      ...
      if (!tasks || !Array.isArray(tasks) || tasks.length === 0) {
        throw new Error("No tasks returned by createMockups endpoint");
      }

      // (B) Poll tasks
      const pollRes = await fetch("/api/printful/pollMockups", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!pollRes.ok) {
        const pollErrData = await pollRes.json();
        throw new Error(pollErrData.error || "Error polling Printful tasks");
      }

      const pollData = await pollRes.json();
      console.log("Poll data:", pollData);

      // We'll assume pollData.mockups is an array with your final mockups
      setMockups(pollData.mockups);
  ...
  // UI logic for previous/next arrows in the celebrity template carousel
